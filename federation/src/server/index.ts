...
export class ManifoldServer extends EventEmitter {
  ...
  readonly wsHandler: WebSocketHandler
  ...
  constructor(config: ManifoldServerConfig) {
    ...
    // WebSocket handler — owns connection lifecycle and message routing
    this.wsHandler = new WebSocketHandler({
      ...
      registerAgent: (name, capabilities, seams) => this.registerAgent(name, capabilities, seams),
    })

    // Forward handler events to server-level listeners
    this.wsHandler.on('agent:join', (agent) => this.emit('agent:join', agent))
    this.wsHandler.on('agent:leave', (agent) => this.emit('agent:leave', agent))
    this.wsHandler.on('peer:connect', (peer) => this.emit('peer:connect', peer))
    this.wsHandler.on('peer:disconnect', (peer) => this.emit('peer:disconnect', peer))
    this.wsHandler.on('mesh:sync', (hub) => this.emit('mesh:sync', hub))
    this.wsHandler.on('mesh:delta', (hub) => this.emit('mesh:delta', hub))
  }
  ...
  async start(): Promise<void> {
    ...
    // 6. Wire peer registry message routing into WebSocketHandler
    this.wsHandler.wirePeerRegistry()

    // 7. Start gossip sampler
    this.peerRegistry.start()

    // 8. Start periodic mesh sync
    ...
    // 10. Start task history
    await this.taskHistory.start()

    // 11. Start metrics collector
    ...
    // 12. Wire detection coordination broadcast
    this.detectionCoord.setBroadcast((msg) => {
      // Broadcast to all local clients via handler
      this.wsHandler.localClients.forEach(ws => {
        if (ws.readyState === WebSocket.OPEN) {
    ...
    // 13. Start cross-hub detection sync
  ...
  private async _startFederationServer(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      // Delegate connection handling to WebSocketHandler
      this.wsHandler.bindFederation(wss)
