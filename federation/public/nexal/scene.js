...
export function buildSpiderWeb() {
  // Clear previous spider web
  if (window._webGroup && window._webGroup.parent) {
    window._webGroup.traverse(child => {
      if (child.geometry) child.geometry.dispose();
      if (child.material) child.material.dispose();
    });
    window._webGroup.parent.remove(window._webGroup);
  }
...
export function buildAgentTopologies(agents) {
  // Clear previous — remove old objects from scene and dispose geometry/materials
...
export function buildCentralNexus() {
  // Clear previous constraint system
  if (window._constraintSystem) {
    const old = window._constraintSystem;
    [old.group, old.particleGroup].forEach(g => {
      if (g && g.parent) {
        g.traverse(child => {
          if (child.geometry) child.geometry.dispose();
          if (child.material) child.material.dispose();
  ...
  if (window._centralSphere && window._centralSphere.parent) {
    window._centralSphere.parent.remove(window._centralSphere);
  }
