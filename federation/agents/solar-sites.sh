...
case "$COMMAND" in
  ping)
    ...
    ;;
  health-check)
    SITES=$(ls /path/to/your/workspace/sites/ 2>/dev/null | wc -l || echo 0)
  ...
  status)
    ...
    ;;
  list-sites)
    SITES=$(ls /path/to/your/workspace/sites/ 2>/dev/null | head -10 | python3 -c "import sys,json; print(json.dumps(sys.stdin.read().split()))" 2>/dev/null || echo '[]')
