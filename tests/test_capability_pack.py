...
class TestLoadAll:
    def test_load_all_with_agent(self, builder, agent):
        ...
        assert len(specs) == 65  # +5 tool-use +5 collaboration +5 subscription +7 trust pack
    ...
    def test_load_all_without_agent(self, builder):
        ...
        assert len(specs) == 58  # +5 tool-use +5 collaboration +5 subscription +7 trust pack
    ...
    def test_stats_after_loading(self, builder, agent):
        load_all_packs(builder, agent)
        stats = builder.stats()
        assert stats["total_capabilities"] == 65
        assert stats["active"] == 65
